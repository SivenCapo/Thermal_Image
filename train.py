...
dataset_path = "/jsw/fusion/Thermal-Image-Enhancement/MSRS"
#dataset_path = "/jsw/fusion/MMIF-CDDFuse/image/"
scale_factor = 2
num_epochs = 100
output_dir = "./result_jsw"
	...
	for epoch in range(1,num_epochs+1):
		...
		for patches,gt in dataset(dataset_path,batch_size,scale_factor):
			...
			if cuda:
				...
				patches = 0.299 * patches[:, 0, :, :] + 0.587 * patches[:, 1, :, :] + 0.114 * patches[:, 2, :, :]
				gt = 0.299 * gt[:, 0, :, :] + 0.587 * gt[:, 1, :, :] + 0.114 * gt[:, 2, :, :]
		...
		# Validation
		model.eval()
		val_img_file = random.choice([f for f in os.listdir(dataset_path) if f.endswith('.png')])
